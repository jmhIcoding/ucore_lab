...
sfs_io_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, void *buf, off_t offset, size_t *alenp, bool write) {
  ...
  //LAB8:EXERCISE1 YOUR CODE HINT: call sfs_bmap_load_nolock, sfs_rbuf, sfs_rblock,etc. read different kind of blocks in file
	 ...
	 *               Rd/Wr size = (nblks != 0) ? (SFS_BLKSIZE - blkoff) : f(endpos - offset)
	...
	*/
    ...
    uint32_t i = 0;
    for (i = 0; i < nblks; i ++) { 
        if ((ret = sfs_bmap_load_nolock(sfs, sin, blkno + i, &ino)) != 0) goto out;
        if ((ret = sfs_block_op(sfs, buf + alen, ino, 1)) != 0) goto out;
        alen += SFS_BLKSIZE;
    }
    if (endpos % SFS_BLKSIZE != 0) {
        if ((ret = sfs_bmap_load_nolock(sfs, sin, blkno + nblks, &ino)) != 0) goto out;
        if ((sfs_buf_op(sfs, buf + alen, endpos % SFS_BLKSIZE, ino, 0)) != 0) goto out;
