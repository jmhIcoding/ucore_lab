...
load_icode(int fd, int argc, char **kargv) {
    ...
    if ((ret = load_icode_read(fd, (void*)elf, sizeof(struct elfhdr), 0)) != 0) goto bad_elf_cleanup_pgdir;
    ...
    uint32_t i;
    off_t ph_off = elf->e_phoff;
    for (i = 0; i < elf->e_phnum; i ++, ph_off += sizeof(struct proghdr)) {
        if ((ret = load_icode_read(fd, (void*)ph, sizeof(struct proghdr), ph_off)) != 0) 
            goto bad_elf_cleanup_pgdir;
    ...
    uintptr_t stacktop = USTACKTOP - argv_size - sizeof(char *) * argc - sizeof(int);
    uintptr_t s = USTACKTOP - argv_size;
    uintptr_t v = stacktop + sizeof(int);
    
    for (i = 0; i < argc; i ++) {
        *(char*)(v) = strcpy((char*)(s), kargv[i]);
        v += sizeof(char *);
        s += strnlen(kargv[i], EXEC_MAX_ARG_LEN) + 1;
    }
